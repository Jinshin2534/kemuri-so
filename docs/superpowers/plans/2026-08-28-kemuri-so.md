...
Run: `pnpm test`
Expected: PASS（ここまでの合計 96 tests）
...
face-api のライブラリ本体も 1.3MB ある。静的に import するとホーム画面でも読み込まれてしまうので、
**`import('@vladmandic/face-api')` を `loadModels` の中で動的に呼ぶ**。
こうすると Vite が別チャンクに切り出し、判定に入るまで取りに行かなくなる。
...
- `pnpm test` が全て通る（`src/lib` の純粋関数、96件）
- `pnpm build` が通る
